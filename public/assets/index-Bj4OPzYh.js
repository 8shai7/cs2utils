...
cl_crosshair_recoil 0`;function Z(e,t,n=``){e&&(e.textContent=t,e.className=`status${n?` ${n}`:``}`)}function ao(e){Ia(e),ka.innerHTML=`
  ...
  `,e)ms?.classList.add(`hidden`),Z(Ds,`Done — set your sensitivity to ${J(t,3)} and play a few sessions before changing again.`,`ok`);else{let{lower:e,higher:t}=ue($);ms?.classList.remove(`hidden`),hs.textContent=String($.round),gs.style.width=`${($.round-1)/7*100}%`,ys.textContent=J(e,3),bs.textContent=J(t,3),Z(Ds,`Round ${$.round} of 7: test both values, then pick the side that feels better.`,``)}$.choices.length>0?(Es?.classList.remove(`hidden`),Es.innerHTML=`<strong>History:</strong><br />${$.choices.map(e=>`Round ${e.round}: chose <strong>${e.side}</strong> (${J(e.lower,3)} vs ${J(e.higher,3)})`).join(`<br />`)}`):(Es?.classList.add(`hidden`),Es.innerHTML=``)}function ks(){let e=Number(fs.value);if(!Number.isFinite(e)||e<=0){Z(Ds,`Enter a valid starting sensitivity greater than 0.`,`error`);return}$=le(e),Os()}function As(e){!$||de($)||($=me($,e),Os())}ps?.addEventListener(`click`,ks),_s?.addEventListener(`click`,()=>As(`lower`)),vs?.addEventListener(`click`,()=>As(`higher`)),xs?.addEventListener(`click`,()=>{$&&($=he($),Os())}),Ss?.addEventListener(`click`,()=>{$=null,Os(),Z(Ds,`Enter a starting sensitivity and press Start PSA.`,``)}),Aa.innerHTML=Ma.map(e=>`<option value="${e.id}">${e.label}</option>`).join(``),Aa.addEventListener(`change`,()=>Ia(Na)),Ia(null),so(),mo();function js(e){return String(e||``).replaceAll(`&`,`&amp;`).replaceAll(`"`,`&quot;`)}function Ms(e){let t=document.querySelector(`#donate-section`),n=document.querySelector(`#donate-actions`);if(!t||!n)return;let r=[];e.paypalUrl&&r.push(`<a class="btn donate-btn paypal" href="${js(e.paypalUrl)}" target="_blank" rel="noopener noreferrer">${Ta}<span>Donate via PayPal</span></a>`),e.steamTradeUrl&&r.push(`<a class="btn donate-btn steam" href="${js(e.steamTradeUrl)}" target="_blank" rel="noopener noreferrer">${Ea}<span>Donate Steam skins</span></a>`),n.innerHTML=r.join(``),t.classList.toggle(`hidden`,r.length===0);let i=document.querySelector(`#donate-fab`);if(i){let t=[];e.paypalUrl&&t.push(`<a class="donate-fab-btn paypal" href="${js(e.paypalUrl)}" target="_blank" rel="noopener noreferrer" title="Donate via PayPal">${Ta}<span>PayPal</span></a>`),e.steamTradeUrl&&t.push(`<a class="donate-fab-btn steam" href="${js(e.steamTradeUrl)}" target="_blank" rel="noopener noreferrer" title="Donate Steam skins">${Ea}<span>Steam</span></a>`),i.innerHTML=t.length?`<span class="donate-fab-label">Support AimKit</span>${t.join(``)}`:``,i.classList.toggle(`hidden`,t.length===0)}}async function Ns(){try{Ms(await _.settings.get())}catch{Ms({paypalUrl:``,steamTradeUrl:``})}}document.addEventListener(`aimkit:settings-updated`,Ns),document.querySelector(`#contact-open`)?.addEventListener(`click`,sa),Je();var Ps=new URLSearchParams(window.location.search).get(`reset`);if(Ps){We(Ps);let e=new URL(window.location.href);e.searchParams.delete(`reset`),window.history.replaceState({},``,e)}var Fs=new URLSearchParams(window.location.search);if(Fs.get(`token`)){Ce(Fs.get(`token`)),je();let e=new URL(window.location.href);e.searchParams.delete(`token`),window.history.replaceState({},``,e)}else if(Fs.get(`steam`)===`linked`){je();let e=new URL(window.location.href);e.searchParams.delete(`steam`),window.history.replaceState({},``,e)}else if(Fs.get(`steam_error`)){let e=new URL(window.location.href);e.searchParams.delete(`steam_error`),window.history.replaceState({},``,e)}Ln(),sr(),Jr(),_i(),Ii(),wr(),aa(),Ns();