...
`;var Oa=document.querySelector(`#preview-canvas`),ka=document.querySelector(`#preview-stats`),Aa=document.querySelector(`#preview-res`),ja=document.querySelector(`#preview-res-scale`),Ma=`ingame`;function Na(){let e=Ma===`fullscreen`?1080:280;Oa.width!==e&&(Oa.width=e,Oa.height=e),Oa.style.imageRendering=Ma===`fullscreen`?`auto`:`pixelated`}Na();var Pa=[{id:`1920x1080`,h:1080,label:`1920 × 1080 (16:9)`},{id:`2560x1440`,h:1440,label:`2560 × 1440 (16:9)`},{id:`3840x2160`,h:2160,label:`3840 × 2160 (4K)`},{id:`1600x900`,h:900,label:`1600 × 900 (16:9)`},{id:`1366x768`,h:768,label:`1366 × 768 (16:9)`},{id:`1280x960`,h:960,label:`1280 × 960 (4:3)`},{id:`1024x768`,h:768,label:`1024 × 768 (4:3)`},{id:`1280x1024`,h:1024,label:`1280 × 1024 (5:4)`}],Fa=null;function Ia(){return Ma===`fullscreen`?1080:(Pa.find(e=>e.id===Aa?.value)||Pa[0]).h}var La=ga({source:Oa,stage:document.querySelector(`.preview-stage`),toggleBtn:document.querySelector(`#magnifier-toggle`),zoomSelect:document.querySelector(`#magnifier-zoom`)});function Ra(e){if(Fa=e,ha(Oa,e,{resHeight:Ia()}),ja)if(e){let t=Pa.find(e=>e.id===Aa?.value)||Pa[0],n=ma(e,t.h);ja.textContent=`≈ ${n.length}px arms · ${n.thickness}px thick @ ${t.h}p`}else ja.textContent=``;La.refresh()}function za(e){Ma=e===`fullscreen`?`fullscreen`:`ingame`,Na(),document.querySelectorAll(`.pmode`).forEach(e=>{let t=e.dataset.pmode===Ma;e.classList.toggle(`active`,t),e.setAttribute(`aria-selected`,String(t))});let t=document.querySelector(`#preview-mode-tag`);t&&(t.textContent=Ma===`fullscreen`?`Relative to full screen`:`Actual in-game size`),Ra(Fa)}document.querySelectorAll(`.pmode`).forEach(e=>e.addEventListener(`click`,()=>za(e.dataset.pmode)));var Y=document.querySelector(`#crosshair-status`),Ba=document.querySelector(`#sensitivity-status`),Va=document.querySelector(`#sharecode-input`),Ha=document.querySelector(`#commands-output`),Ua=document.querySelector(`#commands-input`),Wa=document.querySelector(`#sharecode-output`),Ga=document.querySelector(`#sens-from-game`),X=document.querySelector(`#sens-to-game`),Ka=document.querySelector(`#sens-source`),qa=document.querySelector(`#sens-target`),Ja=document.querySelector(`#sens-source-dpi`),Ya=document.querySelector(`#sens-target-dpi`),Xa=document.querySelector(`#sens-source-myaw`),Za=document.querySelector(`#sens-target-myaw`),Qa=document.querySelector(`#sens-source-yaw`),$a=document.querySelector(`#sens-target-yaw`),eo=document.querySelector(`#source-yaw-field`),to=document.querySelector(`#target-yaw-field`),no=document.querySelector(`#m-yaw-fields`),ro=document.querySelector(`#sens-cm360`),io=document.querySelector(`#sens-stats`),ao=document.querySelector(`#sens-formula`),oo=`CSGO-UseJt-3oTvn-47wPX-hEyER-WZfiK`,so=`cl_crosshairstyle 4
cl_crosshairsize 3
cl_crosshairgap -2
...
cl_crosshair_recoil 0`;function Z(e,t,n=``){e&&(e.textContent=t,e.className=`status${n?` ${n}`:``}`)}function co(e){Ra(e),ka.innerHTML=`
  ...
  `}function lo(e){return e.trim().replace(/\s+/g,``).replace(/^csgo/i,`CSGO`)}function uo(){let e=Va.value.trim();if(!e){Z(Y,`Paste a crosshair share code first.`,`error`);return}let t=lo(e);if(!wa.test(t)){Z(Y,`Invalid format. Expected CSGO-xxxxx-xxxxx-xxxxx-xxxxx-xxxxx`,`error`);return}try{let e=d(t);Va.value=t,Ha.value=se(p(e)),co(e),Z(Y,`Converted share code to console commands.`,`ok`)}catch(e){e instanceof i||e instanceof r?Z(Y,`That share code is not a valid crosshair code.`,`error`):Z(Y,e instanceof Error?e.message:`Failed to decode share code.`,`error`)}}function fo(){let e=Ua.value.trim();if(!e){Z(Y,`Paste crosshair console commands first.`,`error`);return}try{let t=oe(e);Wa.value=f(t),co(t),Z(Y,`Converted commands to share code.`,`ok`)}catch(e){Z(Y,e instanceof Error?e.message:`Failed to encode share code.`,`error`)}}async function po(e,t,n){if(!t){Z(e,`Nothing to copy for ${n}.`,`error`);return}try{await navigator.clipboard.writeText(t),Z(e,`Copied ${n} to clipboard.`,`ok`)}catch{Z(e,`Clipboard access failed. Select and copy manually.`,`error`)}}function mo(){let e=_a[Ga.value]?.supportsMYaw||_a[X.value]?.supportsMYaw;no?.classList.toggle(`hidden`,!e),eo?.classList.toggle(`hidden`,!_a[Ga.value]?.custom),to?.classList.toggle(`hidden`,!_a[X.value]?.custom)}function ho(){let e=Number(Ka.value),t=Number(Ja.value),n=Number(Ya.value),r=Number(Xa.value)||.022,i=Number(Za.value)||.022,a=Number(Qa.value),o=Number($a.value);if(mo(),_a[Ga.value]?.custom&&!(a>0)){Z(Ba,`Enter a valid source custom yaw (° per count).`,`error`);return}if(_a[X.value]?.custom&&!(o>0)){Z(Ba,`Enter a valid target custom yaw (° per count).`,`error`);return}if(!Number.isFinite(e)||e<=0){qa.value=``,ro.textContent=`—`,io.innerHTML=``,ao.textContent=``;return}if(!Number.isFinite(t)||t<=0||!Number.isFinite(n)||n<=0){Z(Ba,`Enter valid DPI values.`,`error`);return}let s=xa({sourceGame:Ga.value,targetGame:X.value,sourceSens:e,sourceDpi:t,targetDpi:n,sourceMYaw:r,targetMYaw:i,sourceCustomYaw:a,targetCustomYaw:o}),c=_a[Ga.value].name,l=_a[X.value].name,u=J(s.targetSensitivity);qa.value=u,ro.textContent=Sa(s.cm360),io.innerHTML=`
  ...
  `,Z(Ba,`${c} → ${l}: ${u}`,`ok`)}function go(){let e=Ga.value;Ga.value=X.value,X.value=e,qa.value&&(Ka.value=qa.value),ho()}function _o(){Ga.value=`cs2`,X.value=`valorant`,Ka.value=`1.25`,Ja.value=`800`,Ya.value=`800`,ho()}var vo={crosshair:`Convert a crosshair share code into console commands, build a code from commands, or design one visually with a live preview.`,sensitivity:`Keep the same cm/360 aim feel across games — with custom yaw values and DPI changes handled for you.`,psa:`Dial in your ideal sensitivity with a guided 7-round A/B test (Perfect Sensitivity Approximation).`,nades:`Browse community grenade line-ups, or sign in to submit your own with a 2D throw guide, videos and photos.`,commands:`Copy up-to-date CS2 launch options and console commands, recommend the ones that help, and share tips in the comments.`,configs:`Share your CS2 configs and video settings, download other players’ setups, and rate the best ones.`,highlights:`Share your best CS2 clips, watch the community’s highlights, and report anything that breaks the rules.`,pros:`Browse pro players’ sensitivity, DPI, resolution and crosshair settings.`,profile:`Your account, contributions, and settings.`,admin:`Moderate content, manage users, sync data sources, and read contact messages.`},yo=document.querySelector(`#tool-desc`);function bo(e){yo&&(yo.textContent=vo[e]||``)}function xo(e){document.querySelectorAll(`.tool-nav .tool-tab`).forEach(t=>{let n=t.getAttribute(`data-tool`)===e;t.classList.toggle(`active`,n),t.setAttribute(`aria-selected`,String(n))}),document.querySelectorAll(`.tool-view`).forEach(t=>{t.classList.toggle(`active`,t.id===`${e}-tool`)}),bo(e),window.scrollTo({top:0,behavior:`smooth`})}document.querySelectorAll(`.tool-nav .tool-tab`).forEach(e=>{e.addEventListener(`click`,()=>xo(e.getAttribute(`data-tool`)))}),document.addEventListener(`aimkit:navigate`,e=>xo(e.detail)),bo(`crosshair`),document.querySelectorAll(`.converter-panel .tab`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-tab`);document.querySelectorAll(`.converter-panel .tab`).forEach(e=>{let n=e.getAttribute(`data-tab`)===t;e.classList.toggle(`active`,n),e.setAttribute(`aria-selected`,String(n))}),document.querySelectorAll(`.converter-panel .tab-panel`).forEach(e=>{e.classList.toggle(`active`,e.getAttribute(`data-panel`)===t)})})}),document.querySelector(`#decode-btn`)?.addEventListener(`click`,uo),document.querySelector(`#encode-btn`)?.addEventListener(`click`,fo),Va.addEventListener(`keydown`,e=>{e.key===`Enter`&&uo()}),Ua.addEventListener(`input`,()=>{let e=Ua.value.trim();if(!e){Ra(null),ka.innerHTML=``;return}try{co(oe(e))}catch{}}),Va.addEventListener(`input`,()=>{let e=lo(Va.value);if(wa.test(e))try{co(d(e))}catch{}}),document.querySelector(`#copy-commands`)?.addEventListener(`click`,()=>{po(Y,Ha.value,`commands`)}),document.querySelector(`#copy-code`)?.addEventListener(`click`,()=>{po(Y,Wa.value,`share code`)}),document.querySelector(`#copy-sharecode-cmd`)?.addEventListener(`click`,()=>{let e=lo(Va.value);if(!e){Z(Y,`Enter a share code first.`,`error`);return}po(Y,`cl_crosshair_sharecode "${e}"`,`import command`)}),document.querySelector(`#load-example-code`)?.addEventListener(`click`,()=>{Va.value=oo,uo()}),document.querySelector(`#load-example-cmd`)?.addEventListener(`click`,()=>{Ua.value=so,fo()});var So={0:[255,255,255],1:[50,250,50],2:[255,255,0],3:[50,50,250],4:[50,250,250]},Q={...te},Co=document.querySelector(`#ed-style`),wo=document.querySelector(`#ed-color`),To=document.querySelector(`#ed-custom-color`),Eo=document.querySelector(`#ed-custom-color-field`),Do=document.querySelector(`#ed-r`),Oo=document.querySelector(`#ed-g`),ko=document.querySelector(`#ed-b`),Ao=document.querySelector(`#ed-rgb-val`),jo=document.querySelector(`#ed-color-swatch`);function Mo(e,t){e&&document.activeElement!==e&&(e.value=String(t))}var No=document.querySelector(`#ed-length`),Po=document.querySelector(`#ed-thickness`),Fo=document.querySelector(`#ed-gap`),Io=document.querySelector(`#ed-outline`),Lo=document.querySelector(`#ed-alpha`),Ro=document.querySelector(`#ed-dot`),zo=document.querySelector(`#ed-tstyle`),Bo=document.querySelector(`#ed-outline-on`),Vo=document.querySelector(`#ed-alpha-on`),Ho=document.querySelector(`#ed-sharecode`),Uo=document.querySelector(`#ed-commands`),Wo=document.querySelector(`#ed-length-num`),Go=document.querySelector(`#ed-thickness-num`),Ko=document.querySelector(`#ed-gap-num`),qo=document.querySelector(`#ed-outline-num`),Jo=document.querySelector(`#ed-alpha-num`),Yo=document.querySelector(`#ed-r-num`),Xo=document.querySelector(`#ed-g-num`),Zo=document.querySelector(`#ed-b-num`),Qo=(e,t,n)=>Math.max(t,Math.min(n,e)),$o=[{key:`length`,slider:No,num:Wo,min:0,max:15},{key:`thickness`,slider:Po,num:Go,min:0,max:6},{key:`gap`,slider:Fo,num:Ko,min:-10,max:10},{key:`outline`,slider:Io,num:qo,min:0,max:3},{key:`alpha`,slider:Lo,num:Jo,min:0,max:255}],es=[{key:`red`,slider:Do,num:Yo},{key:`green`,slider:Oo,num:Xo},{key:`blue`,slider:ko,num:Zo}];function ts(e,t,n){let r=e=>Math.max(0,Math.min(255,Math.round(e))).toString(16).padStart(2,`0`);return`#${r(e)}${r(t)}${r(n)}`}function ns(e){let t=/^#?([0-9a-f]{2})([0-9a-f]{2})([0-9a-f]{2})$/i.exec(e.trim());return t?{r:parseInt(t[1],16),g:parseInt(t[2],16),b:parseInt(t[3],16)}:{r:Q.red,g:Q.green,b:Q.blue}}function rs(){let e=ts(Q.red,Q.green,Q.blue);wo.value=String(Q.color),Mo(To,e);for(let e of es)Mo(e.slider,Q[e.key]),Mo(e.num,Q[e.key]);Ao&&(Ao.textContent=`${Q.red}, ${Q.green}, ${Q.blue}`),jo&&(jo.style.background=e),Eo?.classList.toggle(`hidden`,Q.color!==5)}function is(){for(let e of $o)Mo(e.slider,Qo(Q[e.key],e.min,e.max)),Mo(e.num,Q[e.key])}function as(){Co.value=String(Q.style),Ro.checked=Q.centerDotEnabled,zo.checked=Q.tStyleEnabled,Bo.checked=Q.outlineEnabled,Vo.checked=Q.alphaEnabled,is(),rs()}function os(){is();let e=!Q.outlineEnabled;Io.disabled=e,qo.disabled=e;let t=!Q.alphaEnabled;Lo.disabled=t,Jo.disabled=t;try{Ho.value=f(Q)}catch{Ho.value=``}Uo.value=se(p(Q))}function ss(){co(Q),os()}function cs(){Q.style=Number(Co.value),Q.centerDotEnabled=Ro.checked,Q.tStyleEnabled=zo.checked,Q.outlineEnabled=Bo.checked,Q.alphaEnabled=Vo.checked,ss()}function ls(e){Q[e.key]=Number(e.slider.value),ss()}function us(e,t){let n=Number(e.num.value);if(e.num.value===``||!Number.isFinite(n)){t&&(e.num.value=String(Q[e.key]));return}Q[e.key]=Qo(n,e.min,e.max),t&&(e.num.value=String(Q[e.key])),ss()}function ds(){Q.color=5,Q.red=Qo(Number(Yo.value)||0,0,255),Q.green=Qo(Number(Xo.value)||0,0,255),Q.blue=Qo(Number(Zo.value)||0,0,255),rs(),ss()}function fs(){if(Q.color=Number(wo.value),Q.color!==5){let[e,t,n]=So[Q.color]??So[1];Q.red=e,Q.green=t,Q.blue=n}rs(),ss()}function ps(){Q.color=5,Q.red=Number(Do.value),Q.green=Number(Oo.value),Q.blue=Number(ko.value),rs(),ss()}function ms(){Q.color=5;let{r:e,g:t,b:n}=ns(To.value);Q.red=e,Q.green=t,Q.blue=n,rs(),ss()}$o.forEach(e=>{e.slider.addEventListener(`input`,()=>ls(e)),e.num.addEventListener(`input`,()=>us(e,!1)),e.num.addEventListener(`change`,()=>us(e,!0))}),[Co,Ro,zo,Bo,Vo].forEach(e=>e.addEventListener(`change`,cs)),wo.addEventListener(`change`,fs),To.addEventListener(`input`,ms),To.addEventListener(`change`,ms),es.forEach(e=>{e.slider.addEventListener(`input`,ps),e.num.addEventListener(`input`,ds),e.num.addEventListener(`change`,ds)}),document.querySelector(`#ed-copy-code`)?.addEventListener(`click`,()=>{po(Y,Ho.value,`share code`)}),document.querySelector(`#ed-copy-commands`)?.addEventListener(`click`,()=>{po(Y,Uo.value,`commands`)}),document.querySelector(`#ed-reset`)?.addEventListener(`click`,()=>{Object.assign(Q,te),as(),ss(),Z(Y,`Crosshair reset to defaults.`,`ok`)}),document.querySelector(`.converter-panel .tab[data-tab="visual"]`)?.addEventListener(`click`,ss),as(),os(),Ga.innerHTML=Ca(`cs2`),X.innerHTML=Ca(`valorant`),[Ga,X,Ka,Ja,Ya,Xa,Za,Qa,$a].forEach(e=>{e.addEventListener(`input`,ho),e.addEventListener(`change`,ho)}),document.querySelector(`#sens-swap`)?.addEventListener(`click`,go),document.querySelector(`#copy-sens`)?.addEventListener(`click`,()=>{po(Ba,qa.value,`converted sensitivity`)}),document.querySelector(`#sens-cs2-val`)?.addEventListener(`click`,_o);var hs=document.querySelector(`#psa-start`),gs=document.querySelector(`#psa-begin`),_s=document.querySelector(`#psa-round`),vs=document.querySelector(`#psa-round-num`),ys=document.querySelector(`#psa-bar-fill`),bs=document.querySelector(`#psa-lower`),xs=document.querySelector(`#psa-higher`),Ss=document.querySelector(`#psa-lower-val`),Cs=document.querySelector(`#psa-higher-val`),ws=document.querySelector(`#psa-undo`),Ts=document.querySelector(`#psa-reset`),Es=document.querySelector(`#psa-result`),Ds=document.querySelector(`#psa-result-label`),Os=document.querySelector(`#psa-stats`),ks=document.querySelector(`#psa-history`),As=document.querySelector(`#psa-status`),$=null;function js(){if(!$){_s?.classList.add(`hidden`),ks?.classList.add(`hidden`),Es.textContent=`—`,Ds.textContent=`recommended sensitivity`,Os.innerHTML=``;return}let e=de($),t=e?ge($):fe($);if(Es.textContent=J(t,3),Ds.textContent=e?`final recommended sensitivity`:`current estimate`,Os.innerHTML=`
