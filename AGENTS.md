...
- Pro settings (`server/src/proSettings.js`, Pros tab): seeded from a curated list and browsable. HLTV is Cloudflare-protected and blocks datacenter IPs, so the "Sync from HLTV" action falls back to the seed; point `PRO_SETTINGS_SOURCE_URL` at a reachable JSON feed to auto-update. Same blocked-host caveat as the Valve wiki.
