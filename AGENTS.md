...
- **Frontend (Vite dev server)** — `npm run dev` in the repo root. Serves on `http://localhost:5173/`. The dev server proxies `/api` and `/uploads` to the backend (see `vite.config.js`); override the target with `VITE_API_PROXY` if the backend isn't on `http://localhost:3001`.
- **Backend API (Node.js/Express)** — `npm run dev` in `server/` (uses `node --watch`). Listens on `http://localhost:3001`. Needs a reachable MySQL database (see below). Health check: `GET /api/health`.
- **MySQL** — required by the backend. In this VM a local MariaDB is used for development; production is Hostinger MySQL.

### Database (MySQL / MariaDB)

- The backend reads DB config from `server/.env` (copy from `server/.env.example`). It **auto-creates its tables on startup** (`server/src/db.js`), so no manual migration step is needed.
- Local dev in the VM: MariaDB runs on `127.0.0.1:3306` with database `cs2utils`, user `cs2utils`, password `cs2utils_dev_pw` (matches the committed `server/.env.example` defaults). MariaDB is not managed by systemd here — start it with `sudo mariadbd --user=mysql` if it isn't already running (check `mysqladmin ping` / port 3306).
...
- Whoever registers/logs in with `OWNER_EMAIL` (default `shaital121@gmail.com`, set in `server/.env`) is automatically granted the `owner` role (owner implies admin and cannot be demoted).
- Nade submissions and user-added media are `pending` until an admin approves them; only approved content is public.

### Secrets

- Never commit real DB credentials or `JWT_SECRET`. `server/.env` is gitignored; only `server/.env.example` is tracked. For Cloud Agent runs, put real values in the Secrets panel, not in the repo.

### Build / run

- Frontend scripts (root `package.json`): `npm run dev`, `npm run build` (→ `dist/`), `npm run preview`.
...
- The frontend `src/session.js` store is the single source of truth for auth; the header menu (`src/headerAuth.js`) and the Nades tool (`src/nadesUI.js`) both subscribe to it. Subscribe before doing async loads to avoid missing the initial session-restore event.
